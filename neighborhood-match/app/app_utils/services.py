...
def get_memory_service():
    """Process-wide memory service pointing to Vertex AI Memory Bank in us-central1."""
    bank_id = os.environ.get("MEMORY_BANK_ID", "2602011309562134528")
    if uri := os.environ.get("MEMORY_SERVICE_URI"):
        if uri.startswith("agentengine://"):
            bank_id = uri.replace("agentengine://", "").strip()
        elif not uri.startswith("shared://"):
    ...
    if bank_id:
        ...
        proj = os.environ.get("PROJECT_ID") or os.environ.get("GCP_PROJECT") or os.environ.get("GOOGLE_CLOUD_PROJECT", "")
        if not proj or proj.isdigit():
            proj = "qwiklabs-gcp-04-ea3abe4d2d3b"
