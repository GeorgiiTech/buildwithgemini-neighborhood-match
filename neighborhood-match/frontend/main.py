...
RESOURCE = os.environ.get(
    ...
    "projects/571813420446/locations/us-central1/reasoningEngines/629434672773857280",
)
...
def _extract_parts(parts: list) -> list[dict]:
    ...
    for p in parts:
        ...
        if isinstance(root, TextPart) and getattr(root, "text", None):
            ...
            if not meta and isinstance(data_val, dict):
                meta = data_val.get("metadata")
            ...
            elif isinstance(data_val, dict) and any(
                k in data_val for k in ("beginRendering", "surfaceUpdate", "dataModelUpdate")
            ):
...
async def chat(req: Request):
    ...
    async with httpx.AsyncClient(headers=_auth_headers(), timeout=120) as client:
        ...
        # Check last_task.history for response messages (text and A2UI data parts)
        if last_task is not None:
            for message in reversed(getattr(last_task, "history", None) or []):
                role = getattr(message, "role", None)
                if role in (Role.agent, "agent"):
                    msg_parts = _extract_parts(getattr(message, "parts", []))
                    if msg_parts:
                        for p in msg_parts:
                            if p not in parts:
                                parts.append(p)
